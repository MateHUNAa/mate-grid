...
function Grid:Destory()
     ...
     self.onClick         = nil
     self.onHolding       = nil
     self.onHoldCancelled = nil
     self.onHoldComplete  = nil
     self.customDraw      = nil
     ...
     if dist <= 10.0 then
          for row = 0, self.rows - 1 do
               for col = 0, self.cols - 1 do
                    ...
                    local isBlinking = self.blinkingCell and self.blinkingCell.row == row and
                    self.blinkingCell.col == col
                    ...
                    if isHeld and self.holdProgress and self.holdProgress > 0 then
                         Draw3DText(squarePos.x, squarePos.y, squarePos.z + 0.2, "Holding!", 255, 0, 0, false, 4)
...
function Grid:update()
     ...
     if AimController._aiming then
          ...
          if IsDisabledControlJustPressed(2, 177) and self.hoverCell and not self.isHolding then
               self:handleClick(self.hoverCell, 1)
          end
...
---@param cell GridCell
---@param table table
function Grid:WriteCell2(cell, table)
     for key, val in pairs(table) do
          self:WriteCell(cell, key, val)
     end

     print(("^1Successfully wrote %s at %s:%s with datas:^0\n"):format(self.id, cell.row, cell.col, table),
          json.encode(table, { indent = true }))
end

function Grid:UpdateGridData(key, val)
     if not self then return end

     local oldVal = self[key] or nil
     self[key] = val

     return oldVal, val, key
end

function Grid:ReadGridData(key)
